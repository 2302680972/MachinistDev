...
- 技术找固定且受限: 
    ...
    - 若需实现某功能则应查询SDK进行取证.一切设计必须有事实依据,禁止无依据的猜测!
- 工具使用限制:
    ...
    - 格式特性:
        ...
        - 方法内部规则
            ...
            - 返回值规则:
                ...
                - 在TS视图上,`BeScript({ retVar: "变量名" })`+局部变量区段+每处返回(含最后的隐式返回)均必须保持一致性
                - `// --- 隐式返回勿修改`+`return XXX;`是每个脚本在TS视图上的固定结构.对应游戏当中的语义
        ...
        - 注释规则:
            - 哪里可以使用(常规的)注释:
                ...
                - 方法体有效代码命令当中,使用"//|"表示注释
            - 不可以假设允许自定义注释的常见场景:
                - 局部变量声明区段: 除固定边界标记和返回值分界线外,不支持任何注释.无法写入自定义的注释
