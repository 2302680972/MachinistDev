    ...
    - 格式特性:
        ...
        - 方法内部规则
            ...
            - 返回值规则:
                ...
                - 在TS视图上,`BeScript({ retVar: "变量名" })`+方法体开头连续的`var`声明+每处返回(含末尾隐式返回)均必须保持一致性
        ...
        - 注释规则:
            - 哪里可以使用(常规的)注释:
                ...
                - 方法体内的普通注释直接使用`//`
            - 不可以假设允许自定义注释的常见场景:
                - 方法体开头的`var`声明区: 不支持夹杂自定义注释.`var`声明必须连续放在方法体开头,`var`声明之间不可以插入任何注释或其他代码
...
- 变量/常量规则:
    ...
    - 全局变量
	    ...
	    - 局部变量
	        ...
	        - 只要出现局部变量,就必须连续写在方法体开头,统一使用`var 名称: 类型;`
	        - 返回值也是局部变量.有返回值的方法必然要在方法体开头连续的`var`声明中声明对应返回变量,类型必须与`BeScript({ retVar })`一致
	        - `var`声明之间不可以插入任何注释或其他代码.一旦出现首个非`var`的有效代码,后面就不允许再出现新的局部变量声明
	        - 方法体内部禁止空白行,如需空白可改用"// "
	        - 局部变量禁止和全局变量重名或类型冲突
