...
public Script__积分榜刷新(): void {
  ...
  ret = G.__async(G.dynamicCallRet(BeString.fromBeConst("DanmuGetAllRank"), dict))
  ...
  while (G.float.lt(i, N)) {
    ...
    // 世界排名动图图标
    组编号 = Act.byName<Device_弹幕_54>("弹幕").Script__X_动图_x27_计算组编号(rank.toLong())
    worldRankIcon = itemUI.read<"BeUIRawImage">(BeString.fromBeConst("item/worldRankIcon"))
    Act.byName<Device_弹幕_54>("弹幕").Script__X_动图_x27_应用动图(worldRankIcon, 组编号)
