...
`patch_mech_view` / `patch_mech_view_multiedit` 校验失败时自动保存 `.patch` 草稿文件（unified diff + 基线哈希）。草稿不会干扰 `patch_mech_view` / `patch_mech_view_multiedit` 的正常使用。恢复流程：
1. 读取 patch 文件理解 diff 内容并修正问题
2. 用 `patch_commit_draft` 重新提交或重新`patch_mech_view`/`patch_mech_view_multiedit`从头开始修改
3. 若放弃修改，直接忽略该草稿，不要主动调用 `patch_cleanup_drafts`
