...
3) **刚创建的新机械，不要立刻二次强制移动/旋转**。
- 新建分支要把目标坐标和朝向直接传给 `G.createAIMech` / `G.createPlayerMech`。
- 生成后的短时间内，机械还在完成激活、注入和刚体释放；同一调用栈里再用 `Mech.move` / `Mech.rot` 或 `Device.setPosKinematic` / `Device.setRotKinematic` 覆盖位姿，容易被底层状态覆盖。
- 对象池取出的旧机械不属于“刚创建的新机械”，但仍应保持单一移动口径。
