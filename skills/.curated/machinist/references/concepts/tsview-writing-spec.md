...
- 上述各类拒绝场景中，若文本匹配成功但校验失败，MCP 会自动保存 `.patch` 草稿文件到视图同目录。草稿不会干扰 `patch_mech_view` / `patch_mech_view_multiedit` 的正常使用。若要继续修改，用 `patch_commit_draft` 重新提交；若放弃此次修改，直接忽略该草稿即可，不要主动清理
