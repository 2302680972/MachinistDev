...
### 2.2 局部变量必须走方法体开头连续的 var 声明

方法体里如果需要局部变量：
- 只有存在局部变量时才需要写 `var`；没有返回值且没有局部变量的方法，可以完全没有 `var` 声明；
- 用 `var 名称: Be类型` 声明；
- 有返回值的方法，返回值变量必须出现在方法体开头连续的 `var` 声明里；
- `var` 声明之间不可以插入任何注释或其他代码；
- 一旦出现首个非 `var` 的有效代码，后面就不允许再出现新的局部变量声明；
- 后续只做赋值与调用，不要再新增声明语句。
...
**重要区分**：零件视图中的普通注释直接使用 `//`，代码语句使用完整 TS 语法。
